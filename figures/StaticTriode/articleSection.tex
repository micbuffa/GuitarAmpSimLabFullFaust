...
Figure 1 — Transfer Function Characteristics. Panel (a) shows how the drive parameter $a$ affects the saturation onset; higher $a$ leads to a more compressed and saturated response for the same input level. Panel (b) illustrates the role of the bias $b$; the horizontal shift in the curve causes positive and negative signal peaks to be clipped at different levels, establishing the stage's asymmetry.

Figure 2 — Waveform Deformation. This time-domain view compares symmetrical and asymmetrical clipping. In panel (a), the $b=-0.25$ curve exhibits the characteristic "warm" asymmetric rounding on one side of the waveform. Panel (b) shows how the saturation envelope progressively closes as the drive $a$ is increased from low to high-gain settings.
